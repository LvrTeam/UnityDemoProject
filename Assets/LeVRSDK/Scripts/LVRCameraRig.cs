...
/// </summary>
[ExecuteInEditMode]
public class LVRCameraRig : MonoBehaviour
{
    /// <summary>
    /// The left eye camera.
    /// </summary>
    ...
    /// </summary>
    public event System.Action<LVRCameraRig> UpdatedAnchors;
...
#region Unity Messages
    private void Awake()
    {
        ...
        LVRManager.NativeTextureScaleModified += (prev, current) => { needsCameraConfigure = true; };
        LVRManager.VirtualTextureScaleModified += (prev, current) => { needsCameraConfigure = true; };
        LVRManager.EyeTextureAntiAliasingModified += (prev, current) => { needsCameraConfigure = true; };
        LVRManager.EyeTextureDepthModified += (prev, current) => { needsCameraConfigure = true; };
        LVRManager.EyeTextureFormatModified += (prev, current) => { needsCameraConfigure = true; };
        LVRManager.MonoscopicModified += (prev, current) => { needsCameraConfigure = true; };
        LVRManager.HdrModified += (prev, current) => { needsCameraConfigure = true; };
    }
    ...
    {
        bool monoscopic = LVRManager.instance.monoscopic;

        LVRPose tracker = LVRManager.tracker.GetPose();
        LVRPose hmdLeftEye = LVRManager.display.GetEyePose(LVREye.Left);
        LVRPose hmdRightEye = LVRManager.display.GetEyePose(LVREye.Right);
    ...
    private void UpdateCameras()
    {
        ...
        {
            leftEyeCamera = ConfigureCamera(LVREye.Left);
            rightEyeCamera = ConfigureCamera(LVREye.Right);
    ...
    public void EnsureGameObjectIntegrity()
    {
        ...
        if (leftEyeAnchor == null)
            leftEyeAnchor = ConfigureEyeAnchor(trackingSpace, LVREye.Left);

        if (centerEyeAnchor == null)
            centerEyeAnchor = ConfigureEyeAnchor(trackingSpace, LVREye.Center);

        if (rightEyeAnchor == null)
            rightEyeAnchor = ConfigureEyeAnchor(trackingSpace, LVREye.Right);
...
#if UNITY_ANDROID && !UNITY_EDITOR
            if (leftEyeCamera.GetComponent<LVRPostRender>() == null)
            {
                leftEyeCamera.gameObject.AddComponent<LVRPostRender>();
            }
#endif
        }
...
#if UNITY_ANDROID && !UNITY_EDITOR
            if (rightEyeCamera.GetComponent<LVRPostRender>() == null)
            {
                rightEyeCamera.gameObject.AddComponent<LVRPostRender>();
            }
#endif
        }
    }
    ...
    private Transform ConfigureEyeAnchor(Transform root, LVREye eye)
    {
    ...
    private Camera ConfigureCamera(LVREye eye)
    {
        Transform anchor = (eye == LVREye.Left) ? leftEyeAnchor : rightEyeAnchor;
        Camera cam = anchor.GetComponent<Camera>();

        LVRDisplay.EyeRenderDesc eyeDesc = LVRManager.display.GetEyeRenderDesc(eye);

        cam.fieldOfView = eyeDesc.fov.y;
        cam.aspect = eyeDesc.resolution.x / eyeDesc.resolution.y;
        cam.rect = new Rect(0f, 0f, LVRManager.instance.virtualTextureScale, LVRManager.instance.virtualTextureScale);
        cam.targetTexture = LVRManager.display.GetEyeTexture(eye);
        cam.hdr = LVRManager.instance.hdr;

#if UNITY_ANDROID && !UNITY_EDITOR
        // Enforce camera render order
        cam.depth = (eye == LVREye.Left) ?
        ...
        if (eye == LVREye.Right)
        {
            cam.enabled = !LVRManager.instance.monoscopic;
        }
