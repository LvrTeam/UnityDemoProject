...
/// <summary>
/// Configuration data for LeVR virtual reality.
/// </summary>
public class LVRManager : MonoBehaviour
{
    /// <summary>
    ...
    /// </summary>
    public static LVRManager instance { get; private set; }
    ...
    /// <summary>
    /// Gets a reference to the active LVRDisplay
    /// </summary>
    public static LVRDisplay display { get; private set; }

    /// <summary>
    /// Gets a reference to the active LVRTracker
    /// </summary>
    public static LVRTracker tracker { get; private set; }
    ...
    public static Profile profile
    {
        get {
            ...
            {
#if !UNITY_ANDROID || UNITY_EDITOR
                float ipd = capiHmd.GetFloat(Hmd.LVR_KEY_IPD, Hmd.LVR_DEFAULT_IPD);
                float eyeHeight = capiHmd.GetFloat(Hmd.LVR_KEY_EYE_HEIGHT, Hmd.LVR_DEFAULT_EYE_HEIGHT);
                float[] defaultOffset = new float[] { Hmd.LVR_DEFAULT_NECK_TO_EYE_HORIZONTAL, Hmd.LVR_DEFAULT_NECK_TO_EYE_VERTICAL };
                float[] neckToEyeOffset = capiHmd.GetFloatArray(Hmd.LVR_KEY_NECK_TO_EYE_DISTANCE, defaultOffset);
    ...
    /// <summary>
    /// If true, then the LeVR health and safety warning (HSW) is currently visible.
    /// </summary>
    ...
    /// <summary>
    /// If true, head tracking will affect the orientation of each LVRCameraRig's cameras.
    /// </summary>
    ...
    /// <summary>
    /// If this is true and TimeWarp is true, each LVRCameraRig will stop tracking and only TimeWarp will respond to head motion.
    /// </summary>
    ...
    [NonSerialized]
    private static LVRVolumeControl volumeController = null;
    [NonSerialized]
    ...
    {
        LVRPluginEvent.Issue(RenderEventType.Resume);
    }

    public static void LeaveVRMode()
    {
        LVRPluginEvent.Issue(RenderEventType.Pause);
    }
...
#region Unity Messages
    ...
    private void Awake()
    {
        ...
        {
            OVR_Initialize();
            LVRPluginEvent.Issue(RenderEventType.Initialize);

            ovrIsInitialized = true;
        }

        var netVersion = new System.Version(LVR.Hmd.LVR_VERSION_STRING);
        System.Version ovrVersion = new System.Version("0.0.0");
        var versionString = LVR.Hmd.GetVersionString();
        var success = false;
        try {
            ovrVersion = new System.Version(versionString);
            success = true;
        } catch (Exception e) {
            Debug.Log("Failed to parse LeVR version string \"" + versionString + "\" with message \"" + e.Message + "\".");
        }
        if (!success || netVersion > ovrVersion)
            Debug.LogWarning("Version check failed. Please make sure you are using LeVR runtime " +
                             LVR.Hmd.LVR_VERSION_STRING + " or newer.");
#endif
        ...
        prevEyeTextureAntiAliasing = LVRManager.instance.eyeTextureAntiAliasing;
        prevEyeTextureDepth = LVRManager.instance.eyeTextureDepth;
        prevEyeTextureFormat = LVRManager.instance.eyeTextureFormat;
        prevNativeTextureScale = LVRManager.instance.nativeTextureScale;
        prevVirtualTextureScale = LVRManager.instance.virtualTextureScale;
        prevMonoscopic = LVRManager.instance.monoscopic;
        prevHdr = LVRManager.instance.hdr;

        if (display == null)
            display = new LVRDisplay();
        if (tracker == null)
            tracker = new LVRTracker();
        ...
        if (!LVRUnityVersionChecker.hasD3D9ExclusiveModeSupport && !display.isDirectMode && SystemInfo.graphicsDeviceVersion.Contains("Direct3D 9"))
        {
        ...
        if (!LVRUnityVersionChecker.hasD3D11ExclusiveModeSupport && !display.isDirectMode && SystemInfo.graphicsDeviceVersion.Contains("Direct3D 11"))
        {
    ...
    private void OnApplicationQuit()
    {
        ...
        LeaveVRMode();
        LVRPluginEvent.Issue( RenderEventType.ShutdownRenderThread );
#endif
    }
    ...
    private void OnDisable()
    {
        ...
        {
            OVR_Destroy();
            LVRPluginEvent.Issue(RenderEventType.Destroy);
            _capiHmd = null;
    ...
    private void Start()
    {
        ...
        LVRDisplay.EyeRenderDesc leftEyeDesc = LVRManager.display.GetEyeRenderDesc(LVREye.Left);
    ...
    private void Update()
    {
        ...
        if (volumeController != null)
        {
            ...
            {
                if (gameObject.GetComponent<LVRCameraRig>() != null)
                {
                    volumeControllerTransform = gameObject.GetComponent<LVRCameraRig>().centerEyeAnchor;
                }
            }
...
#if UNITY_ANDROID && !UNITY_EDITOR
            LVRManager.DoTimeWarp(timeWarpViewNumber);
#else
            display.EndFrame();
#endif
        }
    ...
    private static void InitVolumeController()
    {
        if (volumeController == null)
        {
            ...
            GameObject go = GameObject.Instantiate(Resources.Load("LVRVolumeController")) as GameObject;
            if (go != null)
            {
                volumeController = go.GetComponent<LVRVolumeControl>();
            }
            else
            {
    ...
    public void ReturnToLauncher()
    {
        ...
        LVRManager.PlatformUIConfirmQuit();
    }
#endif
    #endregion
    ...
    {
#if UNITY_ANDROID && !UNITY_EDITOR
        LVRPluginEvent.Issue(RenderEventType.PlatformUIConfirmQuit);
#endif
    }
    ...
    {
#if UNITY_ANDROID && !UNITY_EDITOR
        LVRPluginEvent.Issue(RenderEventType.PlatformUI);
#endif
    }
    ...
    {
#if UNITY_ANDROID && !UNITY_EDITOR
        LVRPluginEvent.IssueWithData(RenderEventType.TimeWarp, timeWarpViewNumber);
#endif
    }

    public static void EndEye(LVREye eye)
    {
#if UNITY_ANDROID && !UNITY_EDITOR
        RenderEventType eventType = (eye == LVREye.Left) ?
            RenderEventType.LeftEyeEndFrame :
            RenderEventType.RightEyeEndFrame;
        int eyeTextureId = display.GetEyeTextureId(eye);

        LVRPluginEvent.IssueWithData(eventType, eyeTextureId);
#endif
    }
    ...
    {
#if UNITY_ANDROID && !UNITY_EDITOR
        LVRPluginEvent.Issue(RenderEventType.InitRenderThread);
#endif
    }
