...
//
/// <summary>
/// LVRMainMenu is used to control the loading of different scenes. It also renders out
...
/// have access to.
///
/// LVRMainMenu is currently attached to the LVRPlayerController prefab for convenience,
/// but can safely removed from it and added to another GameObject that is used for general
/// Unity logic.
///
/// </summary>
public class LVRMainMenu : MonoBehaviour
{
    /// <summary>
    ...
    // Handle to LVRPlayerController
    private LVRPlayerController PlayerController = null;
    // Controller buttons
    ...
    private LVRGUI  		GuiHelper 		 = new LVRGUI();
    private GameObject      GUIRenderObject  = null;
    private RenderTexture	GUIRenderTexture = null;

    // We want to use new Unity GUI built in 4.6 for LVRMainMenu GUI
    ...
    private LVRCrosshair Crosshair        	= new LVRCrosshair();
    ...
    void Awake()
    {
        ...
        LVRCameraRig[] CameraControllers;
        CameraControllers = gameObject.GetComponentsInChildren<LVRCameraRig>();

        if(CameraControllers.Length == 0)
            Debug.LogWarning("LVRMainMenu: No LVRCameraRig attached.");
        else if (CameraControllers.Length > 1)
            Debug.LogWarning("LVRMainMenu: More then 1 LVRCameraRig attached.");
        else{
            CameraController = CameraControllers[0];
#if USE_NEW_GUI
            LVRUGUI.CameraController = CameraController;
#endif
        }

        // Find player controller
        LVRPlayerController[] PlayerControllers;
        PlayerControllers = gameObject.GetComponentsInChildren<LVRPlayerController>();

        if(PlayerControllers.Length == 0)
            Debug.LogWarning("LVRMainMenu: No LVRPlayerController attached.");
        else if (PlayerControllers.Length > 1)
            Debug.LogWarning("LVRMainMenu: More then 1 LVRPlayerController attached.");
        else{
            PlayerController = PlayerControllers[0];
#if USE_NEW_GUI
            LVRUGUI.PlayerController = PlayerController;
#endif
        }
    ...
    void Start()
    {
        ...
        // Set the GUI target
        GUIRenderObject = GameObject.Instantiate(Resources.Load("LVRGUIObjectMain")) as GameObject;
        ...
        if(CameraController != null)
        {
            ...
            GridCube = gameObject.AddComponent<LVRGridCube>();
            GridCube.SetLVRCameraController(ref CameraController);
        }

        // Crosshair functionality
        Crosshair.Init();
        Crosshair.SetCrosshairTexture(ref CrosshairImage);
        Crosshair.SetLVRCameraController (ref CameraController);
        Crosshair.SetLVRPlayerController(ref PlayerController);
    ...
    void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.M))
            LVRManager.display.mirrorMode = !LVRManager.display.mirrorMode;
    ...
    void UpdateNewGUIVars()
    {
        ...
        LVRUGUI.strVisionMode = strVisionMode;
#endif
        // Print out FPS
        LVRUGUI.strFPS = strFPS;

        // Don't draw these vars if CameraController is not present
        if (CameraController != null)
        {
            LVRUGUI.strPrediction = strPrediction;
            LVRUGUI.strIPD = strIPD;
            LVRUGUI.strFOV = strFOV;
            LVRUGUI.strResolutionEyeTexture = strResolutionEyeTexture;
            LVRUGUI.strLatencies = strLatencies;
        }

        // Don't draw these vars if PlayerController is not present
        if (PlayerController != null)
        {
            LVRUGUI.strHeight = strHeight;
            LVRUGUI.strSpeedRotationMultipler = strSpeedRotationMultipler;
        }

        LVRUGUI.strRiftPresent = strRiftPresent;
    }
#endif
    ...
    void OnGUI()
    {
        ...
        // Update LVRGUI functions (will be deprecated eventually when 2D renderingc
        // is removed from GUI)
    ...
    void UpdateIPD()
    {
        if(ShowVRVars == true) // limit gc
        {
            strIPD = System.String.Format("IPD (mm): {0:F4}", LVRManager.profile.ipd * 1000.0f);
        }
    }
    ...
    void UpdateVisionMode()
    {
        if (Input.GetKeyDown(KeyCode.F2))
        {
            ...
            LVRManager.tracker.isEnabled = VisionMode;
    ...
    void UpdateFOV()
    {
        if(ShowVRVars == true)// limit gc
        {
            LVRDisplay.EyeRenderDesc eyeDesc = LVRManager.display.GetEyeRenderDesc(LVREye.Left);
    ...
    void UpdateResolutionEyeTexture()
    {
        if (ShowVRVars == true) // limit gc
        {
            ...
            LVRDisplay.EyeRenderDesc rightEyeDesc = LVRManager.display.GetEyeRenderDesc(LVREye.Right);

            float scale = LVRManager.instance.virtualTextureScale;
    ...
    void UpdateLatencyValues()
    {
        ...
        if (ShowVRVars == true) // limit gc
        {
            LVRDisplay.LatencyData latency = LVRManager.display.latency;
    ...
    void UpdateEyeHeightOffset()
    {
        if(ShowVRVars == true)// limit gc
        {
            float eyeHeight = LVRManager.profile.eyeHeight;
    ...
    void UpdateSelectCurrentLevel()
    {
        ...
        if (Scenes.Length != 0
            && (LVRGamepadController.GPC_GetButton(LVRGamepadController.Button.A)
    ...
    bool ShowLevels()
    {
        ...
        bool curStartDown = LVRGamepadController.GPC_GetButton(LVRGamepadController.Button.Start);
    ...
    int GetCurrentLevel()
    {
        ...
        if(LVRGamepadController.GPC_GetButton(LVRGamepadController.Button.Down) == true)
            curHatDown = true;
    ...
    void GUIShowVRVariables()
    {
        ...
        if ((OldSpaceHit == false) && (SpaceHit == true))
        {
            ...
            else
            {
                ShowVRVars = true;
#if USE_NEW_GUI
                LVRUGUI.InitUIComponent = ShowVRVars;
#endif
            }
        }
    ...
    {
        HMDPresent = LVRManager.display.isPresent;
    ...
    void ShowRiftPresentGUI()
    {
        ...
        c.renderMode = RenderMode.WorldSpace;
        c.pixelPerfect = false;
        LVRUGUI.RiftPresentGUI(RiftPresentGUIObject);
#endif
    }
    #endregion

    /// <summary>
    /// Initialize LVRUGUI on OnDestroy
    /// </summary>
    void OnDestroy()
    {
#if USE_NEW_GUI
        LVRUGUI.isInited = false;
#endif
    }
}
